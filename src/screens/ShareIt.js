...
export const ShareIt = ({ navigation, route }) => {
  ...
  return (
    <CenteringView>
      <Text style={{ fontSize: 20, maxWidth: 340, paddingBottom: 20, textAlign: 'center' }}>Please post the Logoed photo to Instagram with the copied caption to enter the raffle!</Text>
      <Button title="Okay! To Instagram!" onPress={() => goToInstagram()}/>
      <Button title="Okay! Posted With Caption!" onPress={() => checkIfWeDidIt()} disabled={pendingGoingToInstagram}/>
    </CenteringView>
  );
}